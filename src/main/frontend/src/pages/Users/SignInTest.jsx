...
import { Avatar, Button,CssBaseline,TextField,FormControlLabel,Checkbox,Paper,Grid,Box,Typography} from "@mui/material";
...
import NaverLoginButton from "../../utils/NaverLoginButton";
...
export default function SignIn() {
  ...
  const CustomCheckbox = ({ checked, onChange }) => (
    <Checkbox
      checked={checked}
      onChange={onChange}
      sx={{ color: "002500", "&.Mui-checked": { color: "#66BB6A" } }}
    />
  );
    ...
    if (!userInfo.email) {
      ...
      login(userInfo).then((res) => {
        if (!res) {
          ...
          axios
            .get(`dp/user/signin`, { params: { email: userInfo.email } })
            .then((res) => {
              ...
              if (res.data.role !== "점주") {
                ...
                localStorage.setItem(
                  "splitAddress",
                  JSON.stringify(
                    splitAddressFromCurrentUserAddress(res.data.currentAddress)
                  )
                );
              }
            })
